...
يلتقط قيمة PMKID من إطار EAPOL M1 واحد ويكسر كلمة مرور WPA2 اختياريًّا على الجهاز. **لا يتطلّب عميلًا** — وهو أخفى من التقاط مصافحة WPA (`ws`).
...
يراقب العنقاء قناة الهدف سلبيًّا. وترسل نقطة الوصول M1 كلّما ارتبط أي عميل طبيعيًّا. وعند العثور على PMKID في بيانات المفتاح (Key Data) تعرض الشاشة:
...
> **ملاحظة:** لا تُضمّن كل الراوترات قيمة PMKID KDE في M1. فإن رأيت `M1 seen — no PMKID in Key Data`، فالراوتر لا يدعم هذا الهجوم — استخدم `ws` بدلًا منه.
