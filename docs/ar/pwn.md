...
2. **يلتقط** المادة القابلة للكسر — مصافحات رباعية كاملة/نصفية (بمساعدة deauth) و PMKID. تُلتقط الـ PMKID **بلا عميل**: يتظاهر الحيوان بالارتباط بكل نقطة وصول ليستدرّ منها رسالة المصافحة الأولى (بلا عميل وبلا deauth). تُحفظ كلها كملفات `.cap` قياسية تعمل في **hashcat _و_ aircrack-ng** معًا — يحتفظ الحيوان برسالة M3 من نقطة الوصول عند ظهورها ليحمل الملف زوج M2+M3 الذي يتطلّبه aircrack-ng، مع طوابع زمنية متدرّجة بشكل صحيح لتقبل الأدوات المصافحة.
...
- تعرض الشاشة `g<N>` (عدد الأقران) على سطر الإحصاءات، ورسالة `met ANQA-XXXX` مع وضعية عنقاء سعيدة عند ظهور حيوان جديد.
...
> ✅ **تم التحقق على العتاد (ثلاثة أجهزة، 2026-08-23).** أُكِّد الاكتشاف المتبادل، والالتقاء عبر قنوات مختلفة (وجدت الأجهزة بعضها خلال ثوانٍ رغم التجوّل على كل الـ13)، واقتسام القنوات الحتميّ (ثلاثة أجهزة شكّلت `cell 3/3` بشكل صحيح وأخذ كل منها مساره)، وكبح التذبذب، والعودة النظيفة إلى منفرد، ومشاركة بيانات الاعتماد من جهاز بلا بطاقة إلى جهاز ببطاقة — كلها على عتاد حقيقي.
...
- **يحفظ الشبكة المكسورة في NVS** — في مخزن شبكات الواي فاي نفسه الذي يقرؤه `connectwifi` (`cw`) و`scanwifi` (`sw`)، فتصبح الشبكة التي تكسرها بلا بطاقة شبكةً محفوظة على الجهاز. (مفاتيح NVS محدودة بـ15 محرفًا، لذا لا يمكن تخزين اسم شبكة أطول من ذلك.)
- **القائمة البيضاء تعمل في الذاكرة** — `pwn wl add/rm/clear/list` كلها تعمل؛ وتُمحى المدخلات عند إعادة التشغيل.
- **الشبكة (Grid) تعمل بالكامل**، بما في ذلك مشاركة بيانات الاعتماد المكسورة.
