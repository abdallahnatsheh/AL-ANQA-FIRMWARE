...
| `undercover` | `uc` | `[EXP]` `[set\|clear\|status\|boot on\|off\|panic set\|off]` | Silent Notes disguise — mutes notifications, hides status bar; secret passphrase exits without saving; `boot on` boots directly into cover; `panic set` arms an instant-hide key (default `@`) |
| **WiFi** | | | |
...
| `isoscan` | `is` | `[ns#] [inject\|portup\|bounce\|portdown\|dns\|auto]` · `cctest` | **[EXP]** ACTIVE isolation bypass (**transmits**) — CCMP-encrypts a broadcast frame with the live GTK + spoofs the AP MAC to reach clients past isolation. `inject` = ARP reachability probe (HW-proven), `portup` = gateway ARP-poison, `bounce` = reach victim via gateway (IP layer), `portdown` = capture victim frames → SD pcap, `dns` = ICMPv6 RA DNS poison + UDP-53 query logger, `auto` = reachability sweep, `cctest` = crypto self-test. Confirms before firing; targets a netspy row. Efficacy is network-dependent (needs an L2-bridging AP; a phone hotspot routes around it). Own networks only |
...
- [x] `isoscan` **[EXP]** — active isolation bypass (AirSnitch Stage 2, transmits): software AES-CCMP-encrypts a broadcast frame with the live GTK + spoofs the AP MAC to inject at clients past isolation. Attacks: GTK ARP inject (HW-proven), gateway ARP-poison, IP-layer gateway bounce, victim capture→SD pcap, ICMPv6 RA DNS poison (+UDP-53 query logger). Interception is network-dependent (needs an L2-bridging AP)
