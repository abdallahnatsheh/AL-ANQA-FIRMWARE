...
| `isoscan` | `is` | `[ns#] [inject\|portup\|bounce\|portdown\|auto]` · `cctest` | **[EXP]** ACTIVE isolation bypass (**transmits**) — CCMP-encrypts a broadcast frame with the live GTK + spoofs the AP MAC to reach clients past isolation. `inject` = ARP reachability probe (HW-proven), `portup` = gateway ARP-poison, `bounce` = reach victim via gateway at IP layer, `portdown` = MAC-spoof downlink theft, `auto` = reachability sweep, `cctest` = crypto self-test. Confirms before firing; targets a netspy row. Efficacy is network-dependent (needs an L2-bridging AP; a phone hotspot routes around it). RA DNS poison deferred to lab. Own networks only |
