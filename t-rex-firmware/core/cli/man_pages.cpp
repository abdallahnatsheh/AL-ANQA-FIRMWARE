...
static const ManEntry PAGES[] = {
    ...
    { "isoscan", "is", {
        ...
        "         bounce via-gw     portdown MAC-steal",
        "         auto reach-sweep  (bcast: see inject)",
        "",
    ...
    { "undercover", "uc", {
        ...
        "         Notes disguise as `notes`, but raises the",
        "         covert flag so the device goes SILENT: notif",
        "         sounds + hidden-SSID beep suppressed, real",
        "         status bar hidden. Passive tools keep logging",
        "         underneath — only the tells go quiet.",
        "",
        "USE      Navigate like `notes` (see `man notes`). Type",
        "         the passphrase anywhere in the cover to exit",
        "         silently — the open note is NOT saved on a",
        "         passphrase exit (keystrokes never touch SD).",
        "         [q] exits only when no passphrase is set.",
        "",
        ...
        "",
        "BOOT     `uc boot on` boots straight into the cover",
        "         (no splash). A lock PIN then fires AFTER the",
        "         passphrase exits Notes. `uc boot off` = normal.",
        "",
