...
static const ManEntry PAGES[] = {
    ...
    { "notes", "nt", {
        "SYNTAX   notes",
        "",
        "ABOUT    [EXP] 'Notes cover' UI — the disguise app",
        "         behind undercover mode. Real, working notes",
        "         app: SD-backed files, keyboard typing, and a",
        "         cursor-addressable editor. Renders an ordinary",
        ...
        "",
        "NOTE     Notes save to /notes/NNN.txt on SD (root, not",
        "         /apps/ — stays disguised on a PC-browsed",
        "         card). No SD → notes work for the session only",
    ...
    { "undercover", "uc", {
        ...
        "         uc set       — set a secret exit passphrase",
        "         uc clear     — remove the exit passphrase",
        "         uc status    — show whether one is set",
        "",
        "ABOUT    [EXP] Drop into undercover mode — the same",
        "         Notes disguise as `notes`, but also raises",
        "         the covert flag so the device goes SILENT:",
        "         notification sounds and the hidden-SSID beep",
        "         are suppressed, and the real status bar",
        "         (GPS/shield) is hidden.",
        "",
        "         Passive tools keep running and logging to SD",
        "         underneath the cover — only the tells (beeps,",
        "         popups, shield icon) go quiet.",
        "",
        "USE      Navigate like `notes` (see `man notes`). Type",
        "         the configured passphrase anywhere in the",
        "         cover to exit silently back to the CLI — the",
        "         open note is NOT saved on a passphrase exit,",
        "         so the phrase's own keystrokes never touch SD.",
        "         [q] only works as a fallback when no",
        "         passphrase is set.",
        "",
        "         Touch: one tap wakes a half-dimmed screen,",
        "         double-tap (within 500ms) wakes a fully-off",
        "         screen — undercover mode only, never on the",
        "         plain terminal.",
        "",
        "NOTE     Passphrase is SHA-256(salt+phrase), stored in",
        "         /config/undercover.conf — never logged, never",
        "         written in plaintext. Panic-chord entry (drop",
        "         into cover mid-command) and duress/decoy are",
        "         still TODO.",
        nullptr
    }},
