...
static const ManEntry PAGES[] = {
    ...
    { "undercover", "uc", {
        ...
        "         notif sounds + hidden-SSID beep suppressed,",
        "         real status bar hidden. Passive tools keep",
        "         running/logging underneath — only tells quiet.",
        "",
        "USE      Navigate like `notes` (see `man notes`). Type",
        "         the configured passphrase anywhere in the",
        "         cover to exit silently — the open note is NOT",
        "         saved on a passphrase exit, so the phrase's",
        "         own keystrokes never touch SD. [q] only works",
        "         as a fallback when no passphrase is set.",
        "",
        "         Touch: one tap wakes a half-dimmed screen,",
        "         double-tap (500ms) wakes a fully-off screen —",
        "         undercover mode only, never on the terminal.",
        "",
        "NOTE     Passphrase is SHA-256(salt+phrase) in",
        "         /config/undercover.conf — never plaintext.",
        "         Panic-chord entry + duress/decoy still TODO.",
        nullptr
    }},
