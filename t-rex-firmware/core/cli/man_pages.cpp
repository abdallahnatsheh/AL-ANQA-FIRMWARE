...
static const ManEntry PAGES[] = {
    ...
    { "undercover", "uc", {
        ...
        "         uc set              — set secret exit passphrase",
        "         uc clear            — remove the passphrase",
        "         uc status           — passphrase + boot-cover state",
        ...
        "         saved on a passphrase exit (phrase keystrokes",
        "         never touch SD). [q] exits only when no",
        "         passphrase is set.",
        "",
        "BOOT     `uc boot on` — device boots straight into the",
        "         Notes cover with no splash screen. If a lock",
        "         PIN is also set, the lock screen fires AFTER",
        "         the passphrase exits Notes (not before).",
        "         `uc boot off` restores normal boot.",
        "",
