...
## State
- [Progress Log](progress_log.md) — **2026-07-05: isoscan/is Stage 2 — FULL attack suite BUILT + COMPILES** (4 commits ff7f4fd/a0aedf2/260afc7/8d1e932, UNPUSHED). `is` = inject(HW✅)/bounce/portup/portdown(→SD pcap)/dns(RA poison→SD csv)/auto/cctest(✅). Only inject+cctest HW-proven; rest compile-ready, UNVERIFIED (user HW-testing now). Interception needs a REAL L2 router w/ client isolation (mobile hotspot L3-routes past it — proven). **▶ RESUME: build "smarter/automatic" L1 smart-auto (probe→recommend, auto-detect L2/L3) + L2 combined-MITM (portup+portdown together) — see progress_log "RESUME HERE".** Prior **2026-07-04: undercover PANIC BUTTON ✅** (`uc panic set|off`, `panic_key` default `@`, instant-hide from anywhere incl. mid-command; re-entrant hook in getKeyboardInput; cover-exit `signalRedraw()` fixes mid-command repaint). Undercover now feature-complete (Phase 2 + boot-cover 044d860 + panic). Decoy/duress + freeze-TX ops-policy = user DROPPED. Prior: mc/NimBLE spoof fix (89765e0/6a50e50)
