...
- **License AGPL-3.0** — matches T-Rex.
...
- **Requires Arduino-ESP32 core ≥ 3.1.1.** T-Rex is `espressif32@7.0.1` = Arduino **2.0.x / IDF 4.4**
...
2. **Blocklist on SD, indexed into PSRAM at boot** (NOT per-query SD reads):
   ...
   - Net: leaner than AdBlocker (no HTTP, SD-sourced, ~1.6MB PSRAM — fine on the Plus's 8MB; mind T-Rex
...
**On T-Rex's PlatformIO + Arduino-2.0.x / IDF-4.4 build: NOT without leaving the precompiled-lib stack.**
...
- **Route 1 — `esp32-arduino-lib-builder` (recommended for a native T-Rex command).** Rebuild the SAME
  ...
  every other tool links a behaviorally-identical lib). **One-time** build; after that T-Rex builds exactly
...
- **Route 2 — Arduino-as-IDF-component.** Restructure T-Rex as an IDF project using arduino-esp32 as a
...
- **Dual-partition boot-switch** (T-Rex + separate usblan IDF app in 2 partitions, a command switches boot
  partition + reboots): zero risk to T-Rex, but **user REJECTED it** ("dual boot not good for a simple
  tool").

## THE TWO SHAPES (there is no "simple AND inside T-Rex" — NAT can't link in Arduino)
- **Shape 1 — one T-Rex firmware, `usblan` a native command.** Needs Route 1 (one-time lib rebuild). Only
...
3. Only if you want it as a native T-Rex command → **Route 1 one-time lib rebuild** (Shape 1).
