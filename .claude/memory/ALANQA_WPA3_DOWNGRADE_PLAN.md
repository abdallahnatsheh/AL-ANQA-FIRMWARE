# Al-Anqa — WPA3 Transition Mode Downgrade Attack Plan
...
Integrates into existing Al-Anqa WiFi attack workflow:
...
Before launching the attack, Al-Anqa tests whether deauth actually works
...
Existing Al-Anqa evil twin clones the target AP including its security.
...
occurs. Al-Anqa captures this in monitor mode simultaneously.
...
(disconnects or moves away), Al-Anqa still has crackable material.
...
## Integration with Existing Al-Anqa Code
...
### New files
- `al-anqa-firmware/attacks/wpa3down.cpp/.h` — main command logic
...
- `al-anqa-firmware/attacks/wpa3down.cpp/.h` — new command
- `al-anqa-firmware/wifi_tools.cpp` — add EAPOL capture, PMKID, HCCAPX
- `al-anqa-firmware/scanwifi.cpp` — add RSN IE parsing, PMF flags
- `al-anqa-firmware/shell.cpp` — register command
