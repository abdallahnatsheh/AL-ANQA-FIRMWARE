...
**Command: `wpa3down` / `w3d`** (WiFi). ✅ **PHASE 1 DONE (2026-07-08) + PHASE 2 BUILT & FIRST-HW-TESTED (2026-07-09) + PHASE 3 BUILT (2026-08-05, static-reviewed, NOT HW-tested). Phase 4 not built.** NOTE: the 2026-07-09 "UNCOMMITTED" batch (directed deauth, 20ms flood, extern sdCardManager) was COMMITTED into the rebrand commit `ae84205` (2026-08-02) — tree is clean; that old note is stale.

## PHASE 3 DONE (2026-08-05) — empirical PMF probe + pre-association-aware flood (static-reviewed, NOT HW-tested)
All in `wifi/attacks/wpa3down/wpa3down.cpp` (no new files; reuses `dot11.h` + the existing `w3dDeauth`/`w3dDeauthDir` builders + roguehs, rule 5b).
- **`w3d probe <idx> <victim-mac>`** = PMF recon only (new subcommand, not a new command — leading `probe` token stripped via `strncasecmp`). Own promiscuous cb `w3dProbeCb` (MGMT|DATA) on the target channel + statics `s_ppData/s_ppReauth/s_ppDeauth` + `s_ppVictim/s_ppBssid`. Flow: baseline 2s → directed+broadcast deauth burst 8×150ms → observe 2.5s. **Verdict** (`enum W3dPmf`): victim emits fresh auth/(re)assoc → `PMF_OFF`; data keeps flowing thru the burst (`postData>=baseData/2`, baseData≥4) → `PMF_ON`; idle/silent → `UNK`. Leaves WiFi idle STA (SD-safe). `w3dShowVerdict()` screen.
- **Client auto-discovery (usability, no MAC typing — added 2026-08-05):** with NO explicit MAC, `w3d`/`w3d probe` now sniff the target AP's channel ~5s for data frames to/from its BSSID → harvest active client STA MACs (`w3dClientCb`+ring→`w3dSniffClients` dedup table, sorted busiest-first) → `w3dDrawClientList` trackball picker (vendor via `ouiVendor` + RSSI): trkbl/Enter pick · `[a]` busiest · `[u]` re-sniff · `[b]` broadcast (no victim) · `[q]` cancel. `w3d auto <idx>` skips the picker and auto-takes the busiest. `auto` token parsed alongside `probe` (any order). `w3dPickClient` returns 1=picked/0=broadcast/-1=cancel. Then probe→verdict→attack as below. Explicit `w3d <idx> <mac>` still skips discovery.
- **Attack integration:** with a victim (typed OR picked) `w3d` auto-runs the PMF probe → verdict screen ("any key start / [q] cancel") → sets flood MODE label. Broadcast-only (`[b]`) = probe skipped (UNK, "may not drop").
- **Pre-association-aware flood:** the attack loop's 20ms deauth now BACKS OFF (400ms) whenever roguehs's probe+auth+assoc counters move (a client is joining our rogue) so the tight TX loop doesn't starve the rogue's auth/assoc/M1 responses (resume-plan #2). Chrome shows `Mode: PRE-ASSOC FLOOD (PMF on)` / `DEAUTH DOWNGRADE (PMF off)` / `PMF unknown`.
- **HONEST (unchanged reality):** does NOT drop an already-connected PMF-required client (no single-radio tool can) — pre-assoc flood only disrupts the victim's NEXT (re)assoc (auth/assoc frames aren't PMF-protected). The probe's value = telling the operator WHETHER PMF is the blocker. Method refs in file header: IEEE 802.11-2016 §12 + eaphammer (already in NOTICES Dragonblood #19; no new code/data → no NOTICES churn).
- Docs updated: man `w3d` (PROBE block + LIMITS reword), command desc, `docs/wpa3down.md` (PMF-probe section + limits table). **NEXT = HW-test the probe verdict + still prove Phase-2 capture on a non-PMF victim.**
