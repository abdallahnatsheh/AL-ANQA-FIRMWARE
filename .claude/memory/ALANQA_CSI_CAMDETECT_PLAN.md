# Al-Anqa — CSI Human Detector & Camera Scanner Feature Plan
...
### Use case
Place Al-Anqa on a table, connected to WiFi, and leave it running.
It detects if a human enters or moves in the room using only the WiFi radio.
Useful during physical pentests — leave Al-Anqa running as a motion sensor
...
```
┌─────────────────────────────────────┐
│ AL-ANQA // CSI DETECT        [q] exit │  ← RED header
├─────────────────────────────────────┤
...
```
┌─────────────────────────────────────┐
│ AL-ANQA // CAM DETECT        [q] exit │  ← RED header
├─────────────────────────────────────┤
...
- `al-anqa-firmware/sensing/csidetect.cpp/.h` — CSI presence detection command
- `al-anqa-firmware/sensing/camdetect.cpp/.h` — Camera OUI detection command
- `al-anqa-firmware/sensing/csi_common.cpp/.h` — Shared CSI init/callback

## Files to Modify

- `al-anqa-firmware/shell.cpp` — register csidetect/cd and camdetect/cm
- `al-anqa-firmware/trackme.cpp` — add OUI match to WiFi probe sniff loop
...
## Coding Standards (same as rest of Al-Anqa)
