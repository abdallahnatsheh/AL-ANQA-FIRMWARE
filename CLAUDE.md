...
- **Attacks:** `inject` (BUILT — GTK broadcast ARP who-has = reachability probe; **HW-PROVEN: victim decrypts + replies**), `portup` (BUILT — gateway ARP-poison, sender_ip=`WiFi.gatewayIP()`; installs in the victim's neighbor table). Inject frame: broadcast FromDS hdr (A1=ff.., **A2=BSSID spoofed** — 80211_tx accepts it while associated, HW-confirmed, A3=our MAC) + LLC/SNAP + ARP → `isoCcmpEncrypt` (PN starts `0x800000000000`, incrementing; `[k]` toggles keyid 1/2) → `esp_wifi_80211_tx(WIFI_IF_STA,...)` ~50fps.
  - `bounce` (BUILT, untested): IP-layer reachability probe — `Ping.ping` (ESP32Ping) the victim. lwip's broadcast ARP is usually relayed by the AP even under "client isolation" (which blocks unicast data, not ARP relay), so the victim answers and the ping gets through = reachable past isolation. (A true gateway-MAC bounce needs lwip static ARP entries — `ETHARP_SUPPORT_STATIC_ENTRIES` is OFF in this build, so `etharp_add_static_entry` doesn't exist — hence the plain-ping approach.)
  ...
  - `auto` (BUILT): runs the one-shot `bounce` probe. `bcast` = honest note only (a to-DS frame needs our PTK, which raw `80211_tx` can't supply). Menu now 7 attacks; `is` = 63/64 commands.
- **Reachability detection:** reads our OWN lwip ARP cache (`etharp_request`+`etharp_find_addr` under `LOCK_TCPIP_CORE`, netdiscover's pattern) — the victim's UNICAST reply lands in the IP stack, NOT the promiscuous cb (only group frames decrypt there), so NEVER use promiscuous during inject (it starves the reply from lwip).
- **HONESTY / efficacy is network-dependent (HW-tested 2026-07-05):** the GTK inject (reaching a victim past isolation) is proven. But traffic INTERCEPTION (MITM/DNS) does NOT work on a **mobile hotspot** — the phone L3-routes uplink by dest-IP and ignores the poisoned L2 gateway (0% ping loss with the poison held), and its DTIM caps broadcast delivery to ~1/25s so no poison can hold. The real target = a **router with L2-bridging "AP/client isolation"** (untested — needs one). RA DNS poison deferred (needs IPv6/UDP stack, absent from firmware, + a proper isolated-router test bed). `mc off` THEN `cw` to pin a stable MAC for attacks. `SD_DIR_ISOSCAN=/apps/isoscan`.
