...
**Undercover mode** (`core/system/undercover/undercover.cpp/h` + `covert.h`) — command **`undercover`/`uc`** [EXP], **Phase 1 of the plan (Phase 1a done)**. Drops the device into a *silent* Notes disguise: `runUndercover()` raises the global **`g_covert`** flag (declared in `covert.h`, defined in `undercover.cpp`, `volatile` — read by background pollers), runs the blocking Notes cover, clears the flag on exit. `g_covert` is the spine of the whole feature: every audible/visible leak checks it. **Sound-leak audit (Phase 1a):** `NotificationManager::notify()` no-ops when `g_covert` (one gate covers wguard/macwatch/espchat + all notif sounds, and suppresses the screen-wake tell); the `hidden_ssid` direct-I2S beep also gates on it. **Visual tells** (wguard shield/popup, real status bar) are already suppressed while the cover is foreground via the Notes UI's `displayManager.setBlocked(true)` (those draws honour `isBlocked()`) — `g_covert` is specifically for AUDIO, which bypasses display blocking. Passive tools keep running + logging to SD under the cover; only the tells go quiet. **NOT yet built:** panic-chord entry (fire mid-command), secret-passphrase exit (still `q`), duress/decoy, `UndercoverManager` intercept model — see `.claude/memory/PLAN-undercover-touch.md`.
