...
- **Attacks:** `inject` (BUILT — GTK broadcast ARP who-has = reachability probe; **HW-PROVEN: victim decrypts + replies**), `portup` (BUILT — gateway ARP-poison, sender_ip=`WiFi.gatewayIP()`; installs in the victim's neighbor table). Inject frame: broadcast FromDS hdr (A1=ff.., **A2=BSSID spoofed** — 80211_tx accepts it while associated, HW-confirmed, A3=our MAC) + LLC/SNAP + ARP → `isoCcmpEncrypt` (PN starts `0x800000000000`, incrementing; `[k]` toggles keyid 1/2) → `esp_wifi_80211_tx(WIFI_IF_STA,...)` ~50fps.
  ...
  - `portdown` (BUILT, untested, DISRUPTIVE): MAC-spoof downlink theft — `esp_wifi_stop`+`esp_wifi_set_mac(victim)`+`esp_wifi_start`+`WiFi.reconnect` (mirrors `MacChanger::applyMac`), promiscuous-counts FromDS frames to the cloned MAC. **Restores the original MAC on every exit** (q). Creates a MAC conflict with the real victim.
  - `auto` (BUILT): runs the one-shot `bounce` probe (inject/portup/portdown are interactive/held). `bcast` = honest note only (a to-DS frame needs our PTK, which raw `80211_tx` can't supply → not separately injectable; reachability is answered by inject/bounce).
  - **RA DNS poison = NOT built** (deliberately deferred to a lab session): IPv6 enable (`WiFi.enableIpV6`) + link-local via `esp_netif_get_ip6_linklocal` + hand-built ICMPv6 RA w/ RDNSS + checksum + a WiFiUDP:53 DNS logger. Checksum/ND-sensitive → must be built HW-in-the-loop, and faces the same isolation return-path wall. IPv6 IS enabled in the framework (`CONFIG_LWIP_IPV6=1`), WiFiUDP + the esp_netif ip6 API exist.
...
`/apps/isoscan/` — active isolation-bypass attack logs/captures from `is` (`SD_DIR_ISOSCAN`; dir created, no writers yet — reserved for future capture/log payloads)
