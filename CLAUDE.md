...
- **NOT yet built:** panic-chord entry (drop into cover mid-command via keyboard chord — needs a non-blocking `UndercoverManager` + a stateful `runNotesUi` refactor), duress/decoy passphrase, boot-cover — see `.claude/memory/PLAN-undercover-touch.md`.
...
`/config/undercover.conf` — undercover secret-passphrase config (`hash=`, `salt=`, `len=` — SHA-256(salt+phrase), never plaintext)
