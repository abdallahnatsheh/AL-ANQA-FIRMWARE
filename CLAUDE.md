...
- **Real, SD-backed notes** (`NoteRec{title, body[], path, dirty}`): `/notes/NNN.txt` at SD **root** (`SD_DIR_NOTES`, deliberately NOT under `/apps/` — looks like an ordinary folder on a PC-browsed card). `loadNotesFromSD()` scans + sorts newest-first by numeric filename; no SD → RAM-only fallback (4 hardcoded seed notes), session works but nothing persists (no crash, graceful degrade); empty SD dir → auto-seeds starter files then rescans. `saveNote()` allocates the next sequential filename for a never-saved note, writes title+body, clears `dirty`, returns `false` on no-SD (surfaced honestly in the UI, see below).
...
`/notes/NNN.txt` — undercover Notes cover real files (`SD_DIR_NOTES`, sequential, never reused) — deliberately at SD root, NOT `/apps/`, so a PC-browsed card shows an ordinary "notes" folder, not a T-REX tool folder. See `core/system/undercover/notes_ui.cpp`.
