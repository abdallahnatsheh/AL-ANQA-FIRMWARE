...
## Commands
System: `help/hlp` `info/inf` `clear/clr` `MATRIX/matrix` `pwrsave/psv` `sleep/slp` `lock/lk` `notes/nt [EXP]` `undercover/uc [EXP]`
...
`/config/undercover.conf` — undercover config (`hash=`, `salt=`, `len=` — SHA-256(salt+phrase), never plaintext; `boot_cover=0|1` — boot directly into Notes cover; `panic_key=<byte>` — instant-hide key, default 64='@', 0=off)
