...
//   psk="plain"              — quoted plain text (Al-Anqa native, Linux accepts)
...
// Global (non-block) lines + the Al-Anqa header are preserved verbatim.
...
// Copy conf → .bak once before Al-Anqa ever modifies the file
...
int appendWpaNetwork(const WifiNetwork& net) {
    ...
    // ── backup existing file before first Al-Anqa modification ─────────────────
    ensureBackup();
    ...
    if (isNew)
        block += "# wpa_supplicant.conf — generated by Al-Anqa\n"
