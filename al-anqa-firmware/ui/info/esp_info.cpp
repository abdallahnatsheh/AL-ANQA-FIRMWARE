...
extern LGFX tft;   // global display — needed for the built-in QR renderer

// GitHub project URL encoded in the ABOUT-page QR code
static const char* ALANQA_REPO_URL = "https://github.com/abdallahnatsheh/AL-ANQA-FIRMWARE";
...
// ── Page 3: About (project QR) ────────────────────────────────────────────────

void ESPInfoPrinter::drawPageAbout() {
    DisplayManager& dm = displayManager;

    iRow(dm, "Project ", "AL-ANQA");
    iRow(dm, "Desc    ", "Pentest firmware");
#ifdef BOARD_TDECK_PLUS
    iRow(dm, "Board   ", "LilyGo T-Deck Plus");
#else
    iRow(dm, "Board   ", "LilyGo T-Deck");
#endif
    iSep(dm);

    // QR code to the GitHub repo — LovyanGFX auto-picks the version that fits
    // the URL and draws a self-contained white quiet-zone square (scannable).
    const int32_t qw = 100;
    const int32_t qx = 10;
    const int32_t qy = dm.getCursorY() + 2;
    tft.qrcode(ALANQA_REPO_URL, qx, qy, qw);

    // caption beside the QR
    const uint16_t cx = qx + qw + 12;
    dm.setTextColor(TFT_CYAN);
    dm.printText("Scan to open", cx, qy + 8);
    dm.printText("the project",  cx, qy + 24);
    dm.printText("on GitHub",    cx, qy + 40);
    dm.setTextColor(0x7BEF);
    dm.printText("Repo:",        cx, qy + 66);
    dm.setTextColor(TFT_YELLOW);
    dm.printText("AL-ANQA-",     cx, qy + 82);
    dm.printText("FIRMWARE",     cx, qy + 98);

    // push the cursor below the QR so the caller's footer prints under it
    dm.setCursor(10, qy + qw + 6);
}
