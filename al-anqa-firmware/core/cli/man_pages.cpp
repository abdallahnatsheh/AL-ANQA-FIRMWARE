...
static const ManEntry PAGES[] = {
    ...
    { "pwn", "pw", {
        ...
        "         pwn basic        (round-robin 1/6/11)",
        "         pwn stealth      (low signature)",
        "         pwn passive      (zero TX)",
        "         pwn fast         (AI, only 1/6/11)",
        "         pwn wl ...       (whitelist)",
        "",
        "ABOUT    Autonomous pwnagotchi pet. Roams",
        "         channels, captures WPA handshakes",
        "         + PMKIDs (clientless PMKID solicit),",
        "         and CRACKS them ON-DEVICE during idle",
        "         (resume cursor + smart guessing).",
        "         PWNED = a real recovered password.",
        "",
        "ROAM     DEFAULT = AI adaptive learner over",
        "         ALL 13 channels (learns your area,",
        "         /apps/pwn/learn.csv). 'basic' = plain",
        "         1/6/11 round-robin. [a] toggles live.",
        "MODES    active  = deauth + PMKID solicit + grid",
        "         passive = sniff-only + grid",
        "         stealth = PMKID solicit, NO grid, dark",
        "GRID     AL-ANQA pets greet each other on",
        "         air (name/score) + SHARE cracked",
        "         creds so the pack auto-saves them.",
        "         TX active/passive; stealth listens",
        "         only. Name: /apps/pwn/grid.conf",
        "NOSD     runs cardless: capture + crack ONE",
        "         HS in RAM off the built-in list,",
        "         cracked WiFi saved to NVS; grid +",
        "         whitelist work (RAM, clears reboot).",
        "WLIST    pwn wl list",
        ...
        "         [s]stats [q]quit",
        "WORDL    drop ONE list in /apps/wordlists/",
        "         (shared); else /apps/pwn/wordlist.txt",
        "         else built-in ~100. Changing it",
        "         re-arms exhausted caps.",
