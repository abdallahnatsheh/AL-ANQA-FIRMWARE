...
static const ManEntry PAGES[] = {
    ...
    { "tz", "tz", {
        ...
        "         tz status        — show current TZ + time (+RTC)",
        "         tz +3 | tz -5   — set UTC offset directly",
        "         tz +3:30        — half-hour offset",
        "         tz set <YYYY-MM-DD> <HH:MM[:SS]>",
        "                          — set the clock manually (local)",
        "",
        ...
        "         No WiFi: GPS used as source (Plus + Pager).",
        "         T-Pager: battery-backed RTC keeps time across",
        "         power-off; NTP/GPS/`tz set` all write to it.",
    ...
    { "pwrsave", "psv", {
        ...
        "         T-Pager: on battery, low-% fires an alert tone,",
        "         critical-% auto deep-sleeps (protects the cell).",
        "",
        ...
        "         (conf) batteryWarnPercent / batteryCritical-",
        "         Percent / batteryAutoSleep — T-Pager alert+sleep",
        "",
    ...
    { "pmkid", "pm", {
        "SYNTAX   pm [passive] <idx|bssid> [ch]",
        "",
        "ABOUT    PMKID capture + on-device crack.",
        "         No client needed. DEFAULT = active:",
        "         spoof-associate to force M1/PMKID.",
        "         'pm passive' = silent no-TX sniff.",
        "",
        "STEPS    1. sw  2. pm <idx>  3. wait",
        "         4. c  to crack on-device",
        "MODES    pm <idx>          active (forces it)",
        "         pm passive <idx>  silent, wait only",
    ...
    { "wpa3down", "w3d", {
        ...
        "         3. pick client  4. wait M2  5. cc",
        "VICTIM   No MAC needed - w3d sniffs the AP's",
        "         clients and shows a trackball picker",
        "         ([a]=busiest, [b]=broadcast). Or give",
        "         a MAC: w3d 3 AA:BB:CC:DD:EE:FF, or",
        "         w3d auto 3 = auto-pick the busiest.",
        "PROBE    w3d probe <idx> = test if the client",
        "         enforces PMF (deauth it, watch for",
        "         re-auth vs data flow). The attack",
        "         also probes first, then picks the",
        "         flood mode from the verdict.",
        ...
        "LIMITS   PMF-required won't drop an ESTABLISHED",
        "         link (pre-assoc flood disrupts re-joins",
        "         only). Pure WPA3 not downgradeable.",
    ...
    { "wps", "wps", {
        ...
        "         wps <idx>        the all-in-one screen",
        "",
        "DOES     One command, everything:",
        "         - decodes the beacon WPS IE: version,",
        "           AP-SETUP-LOCKED, methods, mfr/model/",
        "           device-name leak",
        "         - generates candidate PINs (9 algos:",
        "           pin24/28/32, DLink, ASUS, Airocon...)",
        "           + a laptop reaver/pixiewps attack sheet",
        "         - LIVE-sniffs the WPS EAP-WSC handshake",
        "           (M1/M2/M3) -> pixiewps dump",
        "         [p] = WPS push-button connect (SSID+PSK)",
        "         [q] = stop",
        "",
        "OUT      /apps/wps/ : wps.csv, attack_NNN.txt (PIN",
        "         list + reaver cmds), pixie_NNN.txt, creds.csv",
        "NOTE     The ESP32 can't run the WPS auth itself",
        "         (closed stack: no PIN supply / registrar",
        "         / raw-associate). So it RECONs + CAPTURES",
        "         the handshake; the PIN/password crack is",
        "         done offline (pixiewps/reaver). To catch",
        "         M1-M3 a WPS connect must happen on-air.",
        "         Own networks only.",
        nullptr
    }},
    ...
    { "crack", "cc", {
        ...
        "WLIST    no arg -> picker: built-in / ALL",
        "         *.txt here / TYPE A PATH (' auto-",
        "         completes a file/dir) / a .txt here.",
        "         Or pass a file/dir arg directly.",
        "         built-in (100) always tried last.",
        "KEYS     1-8 pick  a/l page  q cancel/stop",
        "RESUME   [q] out of a slow crack, relaunch",
        "         the same cap+list later -> picks up",
        "         at the saved byte offset, not word 0",
        "         (editing the list re-arms from top).",
    ...
    { "pwn", "pw", {
        "SYNTAX   pwn            (AI roam, 13ch)",
        "         pwn basic      (1/6/11 rrobin)",
        "         pwn stealth | passive | fast",
        "         pwn wl ...     (whitelist)",
        "",
        "ABOUT    Autonomous pwnagotchi pet: roams,",
        "         captures WPA HS + PMKIDs (client-",
        "         less solicit), CRACKS on-device.",
        "         PWNED = a real recovered pass.",
        "         Caps: hashcat + aircrack ready.",
        "",
        "ROAM     DEFAULT AI learner, ALL 13ch",
        "         (learns area, learn.csv). 'basic'",
        "         = 1/6/11. [a] toggles live.",
        "MODES    active=deauth+PMKID solicit",
        "         passive=sniff-only (both social)",
        "         stealth=solicit, dark, NO grid",
        "GRID     2+ decks greet + SHARE cracked",
        "         creds (pack auto-saves). Co-",
        "         located decks SPLIT the 13 chans",
        "         into MAC-sorted lanes (no",
        "         negotiation), covering the band;",
        "         HUD cell R/N. Verified 3 decks.",
        "NOSD     cardless: 1 HS in RAM -> NVS.",
        "WLIST    wl list | add <sw-idx|bssid>",
        "         add ssid <name> | rm <n> | clear",
        "KEYS     [m]mode [c]chans [k]backlog [q]",
        "WORDL    /apps/wordlists (shared)>own>bltn",
    ...
    { "arpspoof", "as", {
        ...
        "         DNS domain, HTTP host, HTTPS SNI) live",
        "         + to /apps/arpspoof/NNN.csv.",
        "",
        "KEYS     [q] stop + heal",
    ...
    { "responder", "rsp", {
        "SYNTAX   rsp            active poison + capture",
        "         rsp passive    listen-only (no reply)",
        "",
        "ABOUT    [EXP] Name-service poisoner + NTLM",
        "         capture. ACTIVE: answers LLMNR/NBT-NS/",
        "         mDNS (incl. wpad) with our IP, then",
        "         fake HTTP(:80)+SMB(:445) auth grabs the",
        "         hash. PASSIVE: only logs the queries it",
        "         sees, transmits nothing. Needs cw.",
        "",
        "CATCH    NetNTLMv2 (-m 5600) + NTLMv1 (-m 5500)",
        "         over HTTP & SMB; HTTP Basic (cleartext);",
        "         serves a WPAD PAC to entice proxy auth.",
        "",
        "OUTPUT   per-session /apps/responder/NNN/ :",
        "         hashes.txt (hashcat) creds.txt (basic)",
        "         captures.csv  queries.csv",
        "",
        "KEYS     [q] stop",
        "NOTE     Captures for OFFLINE cracking (not on",
        "         device). SMB2 best-effort/[EXP]. Own",
        "         networks only.",
        nullptr
    }},
    ...
    { "dpwo", "dw", {
        ...
        "",
        "QUIET    add a service/port to check just ONE",
        "         (less noise): dw <ip> ssh / 554 / http",
        "         list: dw <ip> ftp,telnet",
        "CUSTOM   svc:port hits a protocol on a non-",
        "         standard port: dw <ip> ssh:2222",
        "         dw <ip> http:8443",
        "",
        "PORTS    FTP(21) SSH(22) Telnet(23) HTTP(80/",
        "         81/8000/8080 Basic+Digest) RTSP(554,",
        "         cameras) Redis(6379) MQTT(1883)",
        "         SNMP(161 comm). SSH is slow (per-cred",
        "         key exchange).",
        "",
        "TARGET   an IP, or nd#/ns# from a prior",
        "         netdiscover / netspy scan.",
        "CREDS    built-in list + SD /apps/dpwo/creds.csv",
        "         (user,pass) for all but SSH; SSH uses",
    ...
    { "usbmsc", "um", {
        ...
        "         All SD access on Al-Anqa is blocked",
    ...
    { "jiggle", "jg", {
        ...
        "         BLE: run jg ble, pair AL-ANQA-KBD on",
    ...
    { "usbexec", "ux", {
        ...
        "",
        "REMOTE   Starts SoftAP (default SSID=AL-ANQA-CMD)",
