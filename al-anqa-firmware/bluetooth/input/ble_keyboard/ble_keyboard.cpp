...
void BleKeyboard::start() {
    ...
    auto drawPhase1 = [&]() {
        ...
        dm.setTextColor(TFT_WHITE); dm.println("Advertising as: AL-ANQA-KBD");
        dm.setCursor(10, dm.getCursorY());
...
void BleKeyboard::beginHid(bool bond, const char* cloneMacStr, uint8_t cloneType,
    ...
    // custom name too, else it reverts to AL-ANQA-KBD on connect and exposes the clone.
    NimBLEDevice::setDeviceName((cloneName && *cloneName) ? cloneName : "AL-ANQA-KBD");
    ...
    adv->setName((cloneName && *cloneName) ? cloneName : "AL-ANQA-KBD");  // mimic target's name when cloning
    NimBLEDevice::startAdvertising();
}
...
void BleKeyboard::jiggle() {
    ...
    beginHid();   // advertise as AL-ANQA-KBD; host pairs once, then jiggles
    ...
    auto drawScreen = [&]() {
        ...
        dm.setCursor(10, dm.getCursorY());
        if (!connected) {
            dm.setTextColor(TFT_WHITE); dm.println("Advertising: AL-ANQA-KBD");
            dm.setCursor(10, dm.getCursorY());
