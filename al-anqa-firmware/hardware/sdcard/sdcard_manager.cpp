...
void SDCardManager::ensureAppsReadme() {
    ...
    if (!f) return;
    f.println("Al-Anqa Firmware -- /apps folder map");
    f.println("====================================");
    ...
    f.println("wps/        wps         WPS recon reports + PBC-recovered creds");
    ...
    f.println("dpwo/       dw          default-password hits (results.csv) + creds.csv");
...
bool SDCardManager::initializeTDeckStructure() {
    ...
    if (!SD.exists("/config/pwrsave.conf")) {
        ...
        if (f) {
            f.println("# pwrsave.conf — generated by Al-Anqa");
    ...
    if (!SD.exists("/config/macchanger.conf")) {
        ...
        if (f) {
            f.println("# macchanger.conf — generated by Al-Anqa");
